...
static void systeminit_fatfs(void)
{
	...
	FatFs = &FatFs_sysobj;						/* Assign object memory to the FatFs module */

	printf("\n- FatFs module test\n");
	printf("disk_initialize... ");

	if ( (res = disk_initialize())!= FR_OK) {
		printf("fail(%d)\n\n",(int)res);
		return -1;
	}
	printf("done.\n");

	if ( (res = f_getfree(&clust))== FR_OK ) {
		printf("FAT type = %u\n"              ,(alt_16) FatFs_sysobj.fs_type);
		printf("Bytes/Cluster = %lu\n"        ,(alt_32) FatFs_sysobj.sects_clust * 512);
		printf("Number of FATs = %u\n"        ,(alt_16) FatFs_sysobj.n_fats);
		printf("Root DIR entries = %u\n"      ,FatFs_sysobj.n_rootdir);
		printf("Sectors/FAT = %lu\n"          ,FatFs_sysobj.sects_fat);
		printf("Number of clusters = %lu\n"   ,FatFs_sysobj.max_clust - 2);
		printf("FAT base (lba) = %lu\n"       ,FatFs_sysobj.fatbase);
		printf("DIR base (lba,clustor) = %lu\n",FatFs_sysobj.dirbase);
		printf("Data base (lba) = %lu\n"      ,FatFs_sysobj.database);
		printf("%lu kB total disk space.\n"   ,(alt_32)(FatFs_sysobj.max_clust - 2) * (FatFs_sysobj.sects_clust / 2));
		printf("%lu kB available.\n"          ,clust * (FatFs_sysobj.sects_clust / 2));
	} else {
